...
std::string results_to_json(const std::vector<std::pair<int, double>>& results, const DataManager& dm) {
    std::stringstream ss;
    ss << "[";
    for (size_t i = 0; i < results.size(); ++i) {
        ss << "{";
        ss << "\"item_id\": " << dm.get_original_item_id(results[i].first) << ", ";
        ss << "\"similarity\": " << std::fixed << std::setprecision(6) << results[i].second;
        ss << "}";
        if (i < results.size() - 1) {
            ss << ",";
        }
...
std::string metrics_to_json(const MetricsCalculator& calculator, const std::string& model_name) {
    std::stringstream ss;
    ss << "{";
    ss << "\"model\": \"" << model_name << "\", ";
    ss << "\"precision\": " << std::fixed << std::setprecision(4) << calculator.get_average_precision() << ", ";
    ss << "\"recall\": " << std::fixed << std::setprecision(4) << calculator.get_average_recall() << ", ";
    ss << "\"map\": " << std::fixed << std::setprecision(4) << calculator.get_average_map() << ", ";
    ss << "\"ndcg\": " << std::fixed << std::setprecision(4) << calculator.get_average_ndcg();
    ss << "}";
    return ss.str();
}
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "\n--- Pre-calculando metricas y construyendo indices LSH ---" << std::endl;
    ...
    for (int user_idx = 0; user_idx < std::min(num_test_users, data_manager.get_num_users()); ++user_idx) {
        ...
        bpr_metrics_calculator.add_query_result(user_idx, data_manager, bpr_lsh, bpr_gt, 0, 0);
        ...
        srpr_metrics_calculator.add_query_result(user_idx, data_manager, srpr_lsh, srpr_gt, 0, 0);
    }
    ...
    svr.Get("/api/metrics", [&](const httplib::Request&, httplib::Response& res) {
        std::string bpr_json = metrics_to_json(bpr_metrics_calculator, "LSH + BPR (No Robusto)");
        std::string srpr_json = metrics_to_json(srpr_metrics_calculator, "LSH + SRPR (Robusto)");
...
// Nota: He tenido que hacer ajustes en MetricsCalculator para que las funciones get_... sean públicas
// para que el helper metrics_to_json pueda acceder a ellas.
// Deberás modificar tu MetricsCalculator.h para que esos getters sean public.